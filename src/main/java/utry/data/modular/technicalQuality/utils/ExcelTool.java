...
public class ExcelTool<T> {
    ...
    private CellStyle otherStyle; //其他样式
    private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); //日期格式化,默认yyyy-MM-dd HH:mm:ss
    /**
     * 要指定格式化为double类型的数据
     * key为列号，value为数值格式（例如："0_ "为整数，"0.00_ "为保留2位小数）
     */
    private Map<Integer, String> otherFormatMap = new HashMap<>(); //要指定格式化为double类型的数据
    ...
    public ExcelTool(String title, String sheetName){
        this.title = title;
    ...
    public ExcelTool(String title,int colWidth,int rowHeight,Map<Integer, String> formatMap){
        this.colWidth = colWidth;
        this.rowHeight = rowHeight;
        this.title = title;
        this.workbook = new HSSFWorkbook();
        this.otherFormatMap = formatMap;
        this.otherStyle = this.workbook.createCellStyle();
        init(0);
    }

    public ExcelTool(String title,int colWidth,int rowHeight,Map<Integer, String> formatMap, String sheetName){
        this.colWidth = colWidth;
        this.rowHeight = rowHeight;
        this.title = title;
        this.workbook = new HSSFWorkbook();
    ...
    private void init(int styleFlag){
        ...
        this.styleHead.setFillForegroundColor(IndexedColors.GREY_40_PERCENT.getIndex());// 设置单元格的背景颜色为淡蓝色
        this.styleHead.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        this.styleHead.setBorderTop(BorderStyle.THIN);
        this.styleHead.setBorderRight(BorderStyle.THIN);
        this.styleHead.setBorderBottom(BorderStyle.THIN);
        this.styleHead.setBorderLeft(BorderStyle.THIN);
        switch(styleFlag){
            case 1:
                this.styleBody = this.workbook.createCellStyle();
                this.styleBody.setAlignment(HorizontalAlignment.CENTER);// 左右居中
     ...
     * @throws Exception
     */
    public void exportWorkbook(String title,HttpServletResponse response,List<Column> listTpamscolumn, List<T> datas , boolean flag) throws Exception{
        splitDataToSheets(datas, listTpamscolumn,flag,false);
        response.setContentType("application/msexcel;charset=UTF-8");
        response.setCharacterEncoding("utf-8");
        try {
            title = URLEncoder.encode(title, "UTF-8");
            String encodedFileName = new String(title.getBytes("utf-8"), "ISO-8859-1");
     ...
     * @param datas 行内数据
     * @param flag 是否写入行内数据
     * @param list 需要设置为数值类型的列
     * @return
     * @throws Exception
     */
    public void exportWorkbook(String title,HttpServletResponse response,List<Column> listTpamscolumn, List<T> datas , boolean flag,List<Integer> list) throws Exception{
        splitDataToSheets(datas, listTpamscolumn,flag,false);
        response.setContentType("application/msexcel;charset=UTF-8");
        response.setCharacterEncoding("utf-8");
        try {
    ...
    private void splitDataToSheets(List<T> data, List<Column> listTpamscolumn, boolean flag, boolean rowFlag)throws Exception{
        ...
        for(int i = 1; i <= pieces;i++){
            HSSFSheet sheet = this.workbook.createSheet(this.sheetName+i);
            List<T> subList = data.subList((i-1)*maxColumn, i*maxColumn);
            writeSheet(sheet,subList, listTpamscolumn,flag,rowFlag);
        }
        HSSFSheet sheet = this.workbook.createSheet(this.sheetName+(pieces+1));
     ...
     * 创建单元格
     * @param row  Excel对应的行
     * @param tpamscolumn 当前单元格对象 HSSFCellStyle
     * @param v 单元格内容
     * @throws Exception
     */
    public void createCol(HSSFRow row, Column tpamscolumn, T v) throws Exception{
        ...
        if(value[0]!=null) {
            // otherFormatMap非空，则是要指定为数值类型
            if(!CollectionUtils.isEmpty(otherFormatMap)&&otherFormatMap.containsKey(tpamscolumn.getCol())){
                otherStyle.setAlignment(HorizontalAlignment.CENTER);// 左右居中
                otherStyle.setVerticalAlignment(VerticalAlignment.CENTER);// 上下居中
                ...
                String format = otherFormatMap.get(tpamscolumn.getCol());
                otherStyle.setDataFormat(df.getFormat(format));
                //目前只支持double类型
                cell.setCellStyle(otherStyle);
                cell.setCellValue(Double.parseDouble(value[0].toString()));
                return;
            }
            // 设置单元格样式
...
//========上部分是导出excel的使用（生成excel），下部分是解析excel，由于excel导入==================================================================================================================================
     ...
     * 转换成column对象
     *
     * @param headerMap 需要转换的数据
     * @return
     */
    public List<Column> columnTransformer(Map<String, String> headerMap) {
        List<Column> lc = new ArrayList<>();
        if (headerMap != null) {
            final int[] i = {1};
            headerMap.forEach((k, val) -> {
                Column tpamscolumn = new Column();
                tpamscolumn.setId(String.valueOf(i[0]));
                tpamscolumn.setPid("0");
                tpamscolumn.setContent(val);
                tpamscolumn.setFieldName(k);
                lc.add(tpamscolumn);
                i[0]++;
            });
