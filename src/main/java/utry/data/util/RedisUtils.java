...
public class RedisUtils {
     ...
     * @author 余斌
     */
    private boolean setnx(String k, Object v) {
        ...
        if (result) {
            while (!expire(k, 5L)) {
                if (--time >= 0) {
                    System.out.println("设置redis锁的超时时间失败第一次key：" + k);
                }
            }
        }
        return result;
     ...
     * @param k key
     * @param v value
     * @return 一定返回成功加锁，否则一直死循环
     * @author 余斌
     */
    public boolean tryLock(String k, Object v) {
        while (true) {
            if (setnx(k, v)) {
                return true;
            }
